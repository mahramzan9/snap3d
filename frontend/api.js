 ...
 * Talks to the FastAPI at /api/v1/
 */
const BASE = '/api/v1';
...
const AuthAPI = {
  async register(email, password, nickname) {
    ...
    Auth.setUser({ id: data.user_id, email: data.email, plan: data.plan });
    return data;
  },
  ...
  async uploadImages(files, quality = 'standard', onProgress) {
    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.onload = () => {
        ...
        else { const e = JSON.parse(xhr.responseText); reject(new Error(e.detail || 'Upload failed')); }
      };
  ...
  streamProgress(taskId, onProgress, onComplete, onError) {
    ...
    try {
      ...
      ws.onmessage = (evt) => {
        ...
        onProgress(data);
        if (data.status === 'success') { onComplete(data); stop(); }
        else if (data.status === 'failed') { onError(data.phase || 'Failed'); stop(); }
      };
    ...
    function startPolling() {
      ...
      pollTimer = setInterval(async () => {
        ...
        try {
          ...
          onProgress(task);
          if (task.status === 'success') { onComplete(task); stop(); }
          else if (task.status === 'failed') { onError(task.error_msg || 'Failed'); stop(); }
        } catch (e) { onError(e.message); stop(); }
      }, 3000);
    }
    return stop;
  },
...
const ModelsAPI = {
  ...
  async download(modelId, format, filename) {
    ...
    a.download = filename || `snap3d_model.${format}`;
    a.click();
  },
};
...
const PrintersAPI = {
  ...
  status: (id) => apiGet(`/printers/${id}/status`),
  send: (printerId, modelId, opts) => apiPost(`/printers/${printerId}/send`, { model_id: modelId, ...opts }),
};
